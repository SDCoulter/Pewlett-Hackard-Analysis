...
There are plenty of other queries that could be made with the datasets provided that show insight into the company and develop its preparation for the future. The first one I suggest is **comparing the number of retirees with the number of eligible mentors** (using the challenge's birthyear eligibility) **and grouping by department**. This query would look like:
